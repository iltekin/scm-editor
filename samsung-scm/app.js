(() => {
  ...
  async function loadExampleFile(path, b64Key) {
    try {
      $("status").textContent = t("downloadingExample");
      const filename = path.split("/").pop();

      if (b64Key && window[b64Key]) {
        const buf = base64ToArrayBuffer(window[b64Key]);
  ...
  $("loadExampleLink").addEventListener("click", (e) => {
    e.preventDefault();
    loadExampleFile("example/channel_list_UE40ES8000_1201.scm", "SAMPLE_SCM_B64");
  });
  $("loadExampleEditedLink").addEventListener("click", (e) => {
    e.preventDefault();
    loadExampleFile("example/channel_list_UE40ES8000_1201_edited.scm", "SAMPLE_SCM_EDITED_B64");
  });
